...
- 保护用户手写备注和手动清空备注：只有匹配 Apple 默认备注集合的旧数据才会被迁移，manual 备注不覆盖
- 补齐当前系统内置 `com.apple.campo` / Siri 入口的 Apple 默认应用目录和 29 语种备注资源
- 优化 App Grid 使用技巧标题：标题居中显示并删除 29 个语种标题末尾冒号
- 增加 Apple 默认备注迁移 QA，覆盖旧中文默认备注、历史句号变体、Campo 资源完整性和手写备注保护
- 版本号更新为 `7.9.3`，Build 更新为 `20260619.2142`
...
- 修复非中文使用技巧文案在两行高度内显示不完整的问题；27 个非中文语种的 8 条 tips 都改为稳定两行短句，避免第二段动作说明被裁掉
- 版本号保持 `7.9.1`，Build 更新为 `20260614.1454`
...
- “布局初始化”区域保留“未分类 / 智能化初始化分类 / 应用”流程；“自定义布局”区域保留上一个方案、当前方案、恢复、导出和导入
- 新增 `settings.customLayout` 文案键并补齐 29 个语种，保证多语言环境下第二个分区标题不会显示裸 key 或中文
- 版本号更新为 `7.8.31`，Build 更新为 `20260611.1928`

## [7.8.30] — 2026-06-11

- 设置页数据面板新增“布局初始化”选择：可在“未分类”和“智能化初始化分类”之间选择后点击应用
- 新增“重置为未分类”确认弹窗；确认后仅移除应用与标签的分类关系，使应用回到无普通标签状态，不删除应用、标签或备份
- “智能化初始化分类”继续沿用原有应用前确认与备份建议流程
- 版本号更新为 `7.8.30`，Build 更新为 `20260611.1858`
...
- 点击 tag 仍会滚动到对应分组，保留原有显式导航语义
- 新增项目代码治理入口文档，明确唯一源码根目录、Git/QA/发布冻结与构建产物规则
- 版本号更新为 `7.8.23`，Build 更新为 `20260608.2356`
...
- 优化 Quick Search 阴影表现：为独立面板预留阴影外扩空间，避免阴影被窗口边界裁切，并增强浮层感
- 本次只调整 Quick Search 面板视觉层，不改搜索匹配、快捷键、App Grid 入口、Dock 图标或窗口路由逻辑
- 版本号更新为 `7.8.10`，Build 更新为 `20260602.2302`
